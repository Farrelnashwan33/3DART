...
export default function QualityFeatures() {
  ...
  const filteredFeatures = features.filter((feature) =>
    ...
    feature.points.some((point) => point.toLowerCase().includes(searchQuery.toLowerCase()))
  );

  return (
    <section id="quality" className="py-16 px-4 md:py-24 md:px-6 bg-white">
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          <div>
            ...
            <p className="text-slate-500 text-base sm:text-lg mb-8 font-light leading-relaxed">
          ...
          <div className="grid grid-cols-1 gap-6">
            {filteredFeatures.length === 0 ? (
              <div className="py-10 text-center space-y-3 bg-slate-50 rounded-2xl border border-slate-100">
                ...
                <h3 className="text-lg font-bold text-slate-800">Tidak ada fitur ditemukan</h3>
                <p className="text-slate-500 text-xs max-w-xs mx-auto">Kami tidak dapat menemukan jaminan kualitas dengan kata kunci "{searchQuery}".</p>
              </div>
            ) : (
